...
Matrix3f
Transform::GetTransformMatrix() const
{
  Matrix3f transformMatrix = Matrix3f::Translate(m_position) *
                             Matrix3f::Rotate(m_rotation) *
...
void
Transform::Scale(const Vector2f& scale)
{
  m_scale *= scale;
}
...
void
Transform::SetScale(const Vector2f& scale)
{
  m_scale = scale;
}

void
Transform::Translate(const Vector2f& translation)
{
  m_position += translation;
}

Vector2f
Transform::TransformPoint(Vector2f position) const
{
...
Vector2f
Transform::TransformInversePoint(Vector2f position) const
{
  if (m_parent)
    position -= m_parent->TransformPoint(m_position);
  else
