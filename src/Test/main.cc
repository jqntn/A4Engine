...
main()
{
  ...
  SDLCC sdl;
  Window window("Test audio", 1280, 720);
  ...
  while (isOpen) {
    ...
    alSource3f(
      source, AL_POSITION, ambulancePos.x / 100.f, ambulancePos.y / 100.f, 0.f);
    alSource3f(
      source, AL_VELOCITY, velocity.x / 100.f, velocity.y / 100.f, 0.f);

    auto cm = cameraTransform.GetTransformMatrix();
    auto am = ambulanceTransform.GetTransformMatrix();
    auto m = cm * am;

    ambulance.Draw(renderer, am);
